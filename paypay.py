...
class Items(pg.sprite.Sprite):
    ...
    def init(self,xy :tuple[int,int]):
        super().__init__()
        self.vx, self.vy = xy
        self.image = pg.transform.rotozoom(pg.image.load(f"fig/block.png"),0, 0.1)
...
def main():
    ...
    diff_spd = 0.0001 # 加速度
    logg = None
    brooks=pg.sprite.Group() # 岩を保存する
    tree=pg.sprite.Group() # 木を保存する
    ...
    while True:
        ...
        if tmr%500==0:
            ...
            brooks.add(Items((random.choice([180,300,420]),0)))
        ...
        # blocks[]+=Interval.time(tmr,interval_flag)
